...
Installs the Fable 5 Brain skill surface.
Use --target custom --path <agent-skill-root> for Hermes or another runtime
when its official skill root is known.
Set FABLE_5_BRAIN_INSTALL_HOME to test against a temporary home directory.
USAGE
}
...
base_home="${FABLE_5_BRAIN_INSTALL_HOME:-${HOME}}"

target_dir() {
  case "$1" in
    codex) echo "${base_home}/.codex/skills/fable-5-brain" ;;
    claude) echo "${base_home}/.claude/skills/fable-5-brain" ;;
    agents) echo "${base_home}/.agents/skills/fable-5-brain" ;;
    openclaw) echo "${base_home}/.openclaw/skills/fable-5-brain" ;;
    portable) echo "${base_home}/.agent-skills/fable-5-brain" ;;
    custom) echo "${custom_path%/}/fable-5-brain" ;;
  esac
}
...
install_one() {
  ...
  chmod -R go-rwx "${dest}"
  echo "Fable 5 Brain installed to ${dest}"
}

install_gemini() {
  local dest="${base_home}/.gemini/fable-5-brain"
  local loader="${base_home}/.gemini/GEMINI.md"
  install_one "${dest}"
  mkdir -p "$(dirname "${loader}")"
  python - "${loader}" "@./fable-5-brain/GEMINI.md" <<'PY'
...
start = "<!-- fable-5-brain-install:start -->"
end = "<!-- fable-5-brain-install:end -->"
block = f"{start}\n{import_line}\n{end}"
...
PY
  echo "Fable 5 Brain Gemini loader updated at ${loader}"
}
