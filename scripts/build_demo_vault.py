...
def run(args: list[str]) -> None:
    env = {**os.environ, "FABLE_5_BRAIN_DATE": DEMO_DATE}
    subprocess.run([PY, *args], cwd=REPO, check=True, env=env)
...
def main() -> int:
    ...
    if demo.exists():
        shutil.rmtree(demo)
    run(["scripts/scaffold_vault.py", "--client", "sample-vault", "--client-name", "Sample Client", "--owner", "DA", "--out-dir", str(demo_parent)])
