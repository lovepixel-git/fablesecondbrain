...
FORBIDDEN_TEXT_PATTERNS = {
    "local home path": re.compile(rb"/home/(?!claude(?![A-Za-z0-9_.-]))[A-Za-z0-9_.-]+"),  # /home/claude is the documented claude.ai sandbox path quoted by the corpus, not a local leak
...
def main(argv: list[str] | None = None) -> int:
    ...
    scan_source_tree()
    artifacts = [
        build_zip(dist / f"fable-5-brain-template-v{version}.zip", REPO / "assets" / "template-brain", "fable-5-brain-template"),
        build_zip(dist / f"fable-5-brain-sample-vault-v{version}.zip", REPO / "examples" / "sample-vault", "fable-5-brain-sample-vault"),
        build_source_zip(dist / f"fable-5-brain-source-v{version}.zip", version),
    ]
    for artifact in artifacts:
        validate_zip(artifact["path"])
    manifest = {
        "product": "fable-5-brain",
        "version": version,
...
def build_source_zip(out: Path, version: str) -> dict[str, object]:
    ...
    with zipfile.ZipFile(out, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for path in source_files():
            rel = path.relative_to(REPO)
            if not should_skip(rel):
                zf.write(path, (Path(f"fable-5-brain-v{version}") / rel).as_posix())
                entries += 1
    return {"path": out, "entries": entries}
